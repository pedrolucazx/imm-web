...
import { Box, Field, Select as ChakraSelect, Portal, createListCollection } from "@chakra-ui/react";
  ...
  function SelectItem(props, ref) {
    ...
    return (
      <ChakraSelect.Item
        ...
        borderRadius="0"
        fontWeight="medium"
        _highlighted={{ bg: "primary", color: "black" }}
      >
        {children}
...
interface SelectValueTextProps extends Omit<ChakraSelect.ValueTextProps, "children"> {
  children?(_items: CollectionItem[]): React.ReactNode;
}
  ...
  function SelectRoot(props, ref) {
    return (
      <ChakraSelect.Root
        {...props}
        ref={ref}
        positioning={{ sameWidth: true, placement: "bottom", flip: false, ...props.positioning }}
      >
        {props.asChild ? (
          props.children
        ) : (
...
export function Select({ label, error, placeholder, clearable, options, ...props }: SelectProps) {
  const collection = useMemo(() => createListCollection({ items: options }), [options]);

  const select = (
    <SelectRoot invalid={!!error} collection={collection} {...props}>
